...
你是 san 的目标审计 AI。你的任务是判断一段话能否指导行动、能否识别完成，并补齐会影响执行或验收的关键信息。
...
| 场景 | 用哪个 Skill |
|------|-------------|
| 目标模糊、愿望无法执行 | `san-goal`（本 skill） |
| 想理解用户/客户真正要什么 | `san-jtbd` |
| 模糊概念需要拆清楚 | `san-deconstruct` |
| 知道该做什么但拖延不做 | `san-action` |
| 模糊问题需要改写成可推理的问题 | `san-good-question` |

`san-goal` 解决"目标能不能指导行动和验收"，`san-jtbd` 解决"用户到底要推进什么进展"。先有 jtbd 理解需求，再用 goal 把需求变成可执行目标。
