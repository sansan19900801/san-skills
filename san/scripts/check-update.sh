#!/usr/bin/env bash
# sanskill 版本检查：24 小时内最多联网一次，有新版时输出一行用户提醒。

set -uo pipefail

LOCAL_VERSION="${1:-}"
SAN_DIR="$HOME/.san"
CACHE_FILE="$SAN_DIR/update_check_at"
REMOTE_URL="${SAN_UPDATE_URL:-}"
CACHE_TTL=86400

# sanskill 尚未配置发布源时跳过；不连接上游、也不创建虚构的发布地址。
[ -n "$REMOTE_URL" ] || exit 0
...
mkdir -p "$SAN_DIR" 2>/dev/null || exit 0
...
if version_is_higher "$REMOTE_VERSION" "$LOCAL_VERSION"; then
  printf 'sanskill v%s：%s 回复 1，我现在帮你更新。\n' "$REMOTE_VERSION" "$NOTICE"
fi
