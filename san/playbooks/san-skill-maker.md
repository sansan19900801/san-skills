...
需要把生成的 Skill 安装到本地时，直接把生成目录复制到目标平台的技能根目录（例如 `~/.agents/skills/`），或按该平台自身的安装方式操作。本 Skill 不绑定任何特定发行版，也不临时生成另一套安装脚本。
