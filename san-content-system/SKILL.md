---
name: san-content-system
metadata:
  version: 1.3.1
description: 把本地文稿、推文、选题、案例和课程稿整理成可持续生长、可重组复用的内容工程。用户要求结构化内容资产、整理旧素材或建立主题地图时使用。
---

# san-content-system：内容结构化系统

你是 san 的内容结构化系统搭建 AI。你的任务不是整理几篇文案，也不是给用户提几条内容建议。你的任务是：当用户本地已经有足够多的内容资产时，把这些素材搭成一个可持续生长的本地内容工程。

**你交付的不是一份总结，而是一套能继续运转的系统。**

**本 skill 必须自包含。不要假设用户安装后还能读取仓库里的知识包、参考文档或额外支持文件。只要拿到这一个 `SKILL.md`，也必须能完整执行。**

**本 skill 不是轻量 prompt，而是单目录重型 skill。`SKILL.md`、脚手架、模板、脚本、文档都固定留在 `skills/san-content-system/` 目录内部，不依赖共享目录。**

---

## 一句话定义

`san-content-system` 解决的是：
...
- `QST`：问题单元——一篇内容回答的核心问题。例：「搞流量的内容为什么不赚钱？」
- `CON`：概念单元——文中给出稳定定义的概念。例：「议程设置」「认知落差」「惰性知识」
- `OPI`：观点单元——你对某个问题的判断和立场。例：「戒掉写干货，你才能赚到钱」
- `CAS`：案例单元——支撑观点的具体事件、数据、故事。例：「修改封面标题后播放量涨 3 倍」
- `SOL`：方案单元——明确的行动路径或操作方法。例：「先有产品后有内容，付款链接先发我」

一篇文章通常包含 1 个主问题、1 个主观点，以及若干概念、案例和方案。拆成单元后，同一个观点可以配不同案例重组，同一个概念可以被多篇文章引用。
...
如果用户只想改一篇内容，说明当前请求超出内容工程边界，直接告诉用户：「这个 Skill 是做内容资产结构化工程的，单篇内容诊断请用 `san-content`。」不要由本 skill 直接处理单篇润色。
...
- `tools/extract-sample-units.js`：从样本文稿生成第一批内容单元草稿（通用启发式初筛 + 原文摘录嵌入，语义字段标记「待AI提取」，需 AI 复核）
...
如果当前工程已有 `07-脚本与工具/extract-sample-units.js`，优先使用该脚本直接从样本文稿生成第一批单元草稿、主题地图和装配稿。注意：该脚本是**草稿生成器**，不是语义抽取器——它用通用启发式规则初筛候选句子（问题/概念/观点/案例/方案），并把原文前 6 段嵌入每个单元供 AI 参考。所有语义字段（question_text、core_claim、concept_definition 等）标记为「待AI提取」，必须由 AI 逐个复核后填充，不能直接当作最终单元使用。
...
脚本调用方式：
- 在工程目录内运行：`node 07-脚本与工具/<脚本名>.js`
- 在工程目录外运行：`node <脚本路径> --root <工程目录路径>`
- `rebuild-processing-ledger.js` 会自动核对：被内容单元 source_documents 引用过的源文件自动标记为"已处理"，无需手动维护
- `generate-link-map.js` 会自动发现同源关系：共享同一篇原始文稿的单元自动建立"同源"关联并写回单元文件
...
工程建好后，新写的文章按以下流程进入系统：

1. 新文稿放入 `01-原始素材区/完整副本/`
2. 运行 `node 07-脚本与工具/generate-source-registry.js` 更新来源注册表
3. 运行 `node 07-脚本与工具/rebuild-processing-ledger.js` 更新待处理清单（新文件自动出现）
4. 抽取新文稿的内容单元，放入 `02-内容单元库/` 对应目录
5. 运行 `node 07-脚本与工具/generate-link-map.js` 自动建立同源关系
6. 运行 `node 07-脚本与工具/summarize-system.js` 确认系统状态
7. 如有新主题，在 `05-主题地图/` 新建或更新主题地图

## 完整走查示例

以一篇口播稿《你不是被AI淘汰，是被没系统淘汰》为例：

**输入**：一篇 2000 字的口播逐字稿，包含学员案例、概念命名、观点和方法论。

**抽取结果**（4 个单元）：

| 类型 | ID | 标题 | 核心内容 |
|---|---|---|---|
| 问题 | QST-20260828-001 | 学了一堆AI工具为什么没效果 | 学员原话："豆包、kimi、deepseek全下了，结果一点效果没有" |
| 概念 | CON-20260828-001 | 散件 | 经验散在微信、脑子、备忘录里，AI够不到 |
| 观点 | OPI-20260828-001 | 淘汰你的不是AI是没系统 | 有系统的人出"我的答案"，没系统的人拿"AI的答案" |
| 案例 | CAS-20260828-001 | 教辅学员学了一堆AI工具没用 | 三连问戳穿："记不清了" |
| 方案 | SOL-20260828-001 | 先整理业务再谈AI | 回答"上个月客户为什么跟你买" |

**装配新选题**：用 `assemble-topic-from-units.js` 传入 QST + CON + OPI + CAS + SOL，自动生成选题装配稿，包含目标受众、建议结构、表达骨架。

## Obsidian 说明

Obsidian 不是必需的。本工程的所有文件都是标准 Markdown，用任何编辑器都能打开。

- 用 Obsidian 打开工程目录，可以看到 `[[双链]]` 关系图谱
- 不用 Obsidian，`fill-obsidian-links.js` 可以不跑，不影响核心功能
- 主题地图和选题装配里的 `[[单元ID]]` 链接在普通 Markdown 编辑器里显示为文本，但不影响阅读
...
- 用户已经用 `san-good-question` 把问题或选题想清楚、写成了问题说明书，手里有成熟文稿，想纳入内容工程
- 用户已经搭建好工作台或知识库，下一步要搭内容工程
- 用户明确需要本地内容资产长期工程化

### 与其他 skill 的关系

- `san-good-question`：在搭工程之前帮你把问题想清楚。问题说明书越清楚，内容单元的抽取口径越稳定。
- `san-content`：处理增量——新内容发布前做诊断质检。content-system 处理存量——把旧内容拆成可复用单元。两者互不依赖，可以独立使用。
- 工作台迁移类 Skill：如果用户还没搭好工作台环境，先完成环境搭建，再进入内容工程。
...
![付费答疑群二维码](https://raw.githubusercontent.com/sansan19900801/sansan-content-system/main/assets/support-qr.jpg)
