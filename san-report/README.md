...
`san-save` 三件套的最后一块：**只做汇总，不做新诊断**。把同一项目下的存档按时间合并、去重、分类，生成一份能发给合伙人、留档复盘、跟外部顾问对账的报告。
...
```bash
A=~/.agents/skills/san-report/scripts/archive.py
...
```text
/san-report                          # 合并当前项目全部存档
/san-report --since 2026-08-01       # 只汇总某天之后
/san-report --slug proj-a            # 指定项目
...
- `san-save`：写入存档；
- `san-restore`：恢复/搜索单份存档。

三件套各自带一份相同的 `archive.py`，可独立安装且路径规则一致。报告若要发公众号，可配合 `san-wechat-html` 转成微信粘贴版。

## 安装

```bash
npx -y skills add sansan19900801/san-report -g --all
```
