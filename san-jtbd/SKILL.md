...
name: san-jtbd
description: |
  san JTBD 任务澄清。用 Jobs to Be Done 识别具体情境中用户想推进的进展、切换方案的力量与可观察的选择标准，据此优化产品、内容、服务、决策和 AI 提示词。用户问「到底要解决什么」「为什么会选择这个方案」「用 JTBD 重写提示词」时使用。
metadata:
  version: 1.0.1
---
...
| 场景 | 用哪个 Skill |
|------|-------------|
| 模糊愿望需要拆成可执行目标 | `san-goal` |
| 模糊概念需要拆清楚含义 | `san-deconstruct` |
| 模糊问题需要改写成可推理的问题 | `san-good-question` |
| 识别用户/客户真正想推进什么进展 | `san-jtbd`（本 skill） |

`san-goal` 解决"目标能不能指导行动"，`san-jtbd` 解决"用户到底要什么进展"。前者偏执行验收，后者偏需求理解。
...
![付费答疑群二维码](https://raw.githubusercontent.com/sansan19900801/sansan-jtbd/main/assets/support-qr.jpg)
