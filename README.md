...
san-skills 支持将你的本地文件夹变成 Agent 可以稳定调用的知识库，并提供跨对话保存和恢复工作状态的能力。

- 想把自己的资料变成知识库，使用 `/san-knowledge` 建立导航和版本规则，持续查找、收录和调用资料。
- 想跨对话保留工作状态，使用 `/san-save`、`/san-restore` 与 `/san-report`。数据默认保存在你本机的 `~/.san/` 目录。
- 想把散乱的内容和选题整理成可复用的资产，使用 `/san-content-system` 建立内容工程。
